...
namespace CodeStage.AntiCheat.Detectors
{
	...
	public class WallHackDetector : ActDetectorBase
	{
		...
		private void FixedUpdate()
		{
			...
			Vector3 localPosition = rigidPlayer.transform.localPosition;
		...
		private void Update()
		{
			...
			Vector3 localPosition = charControllerPlayer.transform.localPosition;
			if (localPosition.z > 1f)
			{
			...
			if (base.enabled && base.gameObject.activeSelf)
			{
				...
				if (checkWireframe && wfCamera == null)
				{
					...
					else
					{
						...
						{
							shaderTexture = new Texture2D(4, 4, TextureFormat.RGB24, mipChain: false);
						...
						{
							targetTexture = new Texture2D(4, 4, TextureFormat.RGB24, mipChain: false);
		...
		private void StartRigidModule()
		{
			...
			Vector3 localPosition = rigidPlayer.transform.localPosition;
			if (localPosition.z <= 1f && rigidbodyDetections > 0)
			{
				rigidbodyDetections = 0;
			}
		...
		private void StartControllerModule()
		{
			...
			Vector3 localPosition = charControllerPlayer.transform.localPosition;
			if (localPosition.z <= 1f && controllerDetections > 0)
			{
				controllerDetections = 0;
			}
