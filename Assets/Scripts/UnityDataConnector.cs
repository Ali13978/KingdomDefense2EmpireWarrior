...
public class UnityDataConnector : MonoBehaviour
{
	...
	private IEnumerator GetData()
	{
		...
		if (debugMode)
		{
			UnityEngine.Debug.Log("Connecting to webservice on " + connectionString);
		}
		...
		UnityEngine.Debug.Log(elapsedTime + " : " + response);
	...
	private IEnumerator SendData(string ballName, float collisionMagnitude)
	{
		...
		string connectionString = webServiceUrl + "?ssid=" + spreadsheetId + "&sheet=" + statisticsWorksheetName + "&pass=" + password + "&val1=" + ballName + "&val2=" + collisionMagnitude.ToString() + "&action=SetData";
		if (debugMode)
		{
			UnityEngine.Debug.Log("Connection String: " + connectionString);
		}
		WWW www = new WWW(connectionString);
		float elapsedTime = 0f;
		while (!www.isDone)
